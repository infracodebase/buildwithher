...
import { X, Mail, Lock, ArrowRight, Loader2, Shield, AlertTriangle } from "lucide-react";
...
interface ClaimProfileModalProps {
  ...
  /** 'creator' = user just created this profile (localStorage match), 'visitor' = someone visiting an unclaimed profile */
  claimMode: "creator" | "visitor";
  onClaimed: () => void;
}
...
const ClaimProfileModal = ({
  ...
  const submitClaimRequest = async (userId: string, userEmail: string) => {
    ...
    if (error) {
      if (error.code === "23505") {
        toast({ title: "Claim already submitted", description: "You've already requested to claim this profile." });
      } else {
        toast({ title: "Error", description: "Could not submit claim request." });
      }
      return false;
    }

    // Update builder claim_status to pending
    await supabase
      .from("builders")
  ...
  const handlePostAuth = async () => {
    ...
    if (claimMode === "creator") {
      // Case 1: User just created this profile — direct link
      const success = await linkProfile(user.id);
      if (success) {
        toast({ title: "Profile linked!", description: "You can now edit your profile." });
        onClaimed();
      }
    } else {
      // Case 2 & 3: Visitor claiming
      // Check if auth email matches the profile email (trusted signal)
      if (builderEmail && user.email && builderEmail.toLowerCase() === user.email.toLowerCase()) {
        // Trusted email match — auto-link
        const success = await linkProfile(user.id);
        if (success) {
        ...
        // No trusted match — submit claim request for review
        const success = await submitClaimRequest(user.id, user.email || "");
        if (success) {
  ...
  const handleEmailSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // After successful sign-in, handle linking
  ...
  const isVisitorMode = claimMode === "visitor";
  const title = isVisitorMode ? "Claim this profile" : "Edit your profile";
  const subtitle = isVisitorMode
    ? `Verify your identity to claim and manage ${builderName}'s profile.`
              ...
              {claimSubmitted ? (
                ...
                <div className="text-center space-y-4 py-4">
                  ...
                  <p className="text-sm text-muted-foreground leading-relaxed max-w-sm mx-auto">
                    Your claim for <strong>{builderName}</strong>'s profile is under review.
                    We'll verify ownership and notify you once it's approved.
                  </p>
                ...
                <>
                  ...
                  <div className="rounded-xl bg-secondary/40 border border-border/30 p-3.5 space-y-1.5">
                    ...
                    <p className="text-xs text-muted-foreground flex items-center gap-2">
                      <span className="w-1.5 h-1.5 rounded-full bg-accent flex-shrink-0" />
                      {isVisitorMode
                        ? "Ownership will be verified before granting edit access"
