...
const posterStatements = [
  "I'm building my cloud future",
  "I'm building with Build with Her",
  "I'm growing in Cloud and AI",
  "I'm learning and building every day",
  "I'm becoming visible in tech",
];

const BuilderCard = ({ profile, index = 0 }: { profile: BuilderProfile; index?: number }) => {
  const posterStatement = posterStatements[index % posterStatements.length];
  ...
  return (
    <div className="group relative rounded-2xl overflow-hidden aspect-[3/4] cursor-pointer">
      ...
      <div className="relative z-10 h-full flex flex-col items-center justify-between p-6 text-center">
        ...
        {/* Name + poster statement */}
        <div className="space-y-2">
          <h3 className="font-display font-semibold text-sm text-foreground">{profile.name}</h3>
          <p className="text-xs text-foreground/50 italic leading-relaxed max-w-[85%] mx-auto">"{posterStatement}"</p>
        </div>
      </div>
